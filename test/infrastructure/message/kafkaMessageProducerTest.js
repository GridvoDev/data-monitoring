...
describe('KafkaMessageProducer(topic, options) use case test', ()=> {
    ...
    before(done=> {
        ...
        let client = new kafka.Client(
            ...
            "data-monitoring-producer-client");
        let initProducer = new kafka.Producer(client);
        initProducer.on('ready', ()=> {
            initProducer.createTopics(["data-publish"], true, (err, data)=> {
                if (err) {
                    done(err)
                }
                client.refreshMetadata(["data-publish"], (err)=> {
                    if (err) {
                        done(err)
                    }
    ...
    describe('#produce{Topic}Message(message, traceContext, callback)', ()=> {
        context('produce topic message', ()=> {
            it('should return null if no message', done=> {
                let message = null;
                let traceContext = {};
                messageProducer.produceDataPublishTopicMessage(message, traceContext, (err, data)=> {
                    if (err) {
                        done(err);
                    }
            ...
            it('should return data if message is send success', done=> {
                ...
                messageProducer.produceDataPublishTopicMessage(message, traceContext, (err, data)=> {
                    ...
                    consumer.on('message', function (message) {
                        ...
                        data.dataSource.should.be.eql("station-rain-other");
                        data.value.should.be.eql(100);
