...
class Service {
    ...
    removeMonitor(monitorID, traceContext, callback) {
        if (!monitorID) {
            callback(null, false);
            return;
        }
        ...
        function delMonitorFromRepository() {
            return new Promise((resolve, reject) => {
                self._monitorRepository.delMonitorByID(monitorID, traceContext, (err, isSuccess) => {
                    if (err) {
                        reject(err);
                    }
                    resolve(isSuccess);
        ...
        function* removeMonitor() {
            let isSuccess = yield delMonitorFromRepository();
            return isSuccess
        };
        co(removeMonitor).then((isSuccess) => {
            callback(null, isSuccess);
        }).catch(err => {
            callback(err);
        });
