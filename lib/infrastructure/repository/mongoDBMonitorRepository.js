...
class Repository {
    ...
    save(monitor, traceContext, callback) {
        ...
        mongoClient.then(({db, collection})=> {
            let {monitorID, dataType, rules, monitoringItems}=monitor;
    ...
    getMonitorByID(monitorID, traceContext, callback) {
        let mongoClient = createMongoZipkinClient({
            tracer,
            traceContext,
            dbName: this._dbName,
        ...
        mongoClient.then(({db, collection})=> {
            collection.findOne({monitorID}, {limit: 1}, (err, document)=> {
                    if (err) {
                        callback(err, null);
                        db.close();
                        return;
    ...
    delMonitorByID(monitorID, traceContext, callback) {
        let mongoClient = createMongoZipkinClient({
            tracer,
            traceContext,
            dbName: this._dbName,
        ...
        mongoClient.then(({db, collection}) => {
            collection.deleteOne({monitorID},
                {limit: 1},
                (err, result) => {
                    if (err) {
                        callback(err);
